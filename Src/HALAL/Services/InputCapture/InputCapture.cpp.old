...
InputCapture::Instance::Instance(
    Pin& pin,
    TimerPeripheral* peripheral,
    uint32_t channel_rising,
    uint32_t channel_falling
)
    : pin(pin), peripheral(peripheral), channel_rising(channel_rising),
...
uint8_t InputCapture::inscribe(Pin& pin) {
    if (not available_instances.contains(pin) || pin.mode != OperationMode::NOT_USED) {
        ErrorHandler(
            " The pin %s is already used or isn t available for InputCapture usage",
            pin.to_string().c_str()
        );
        return 0;
    }
    ...
    vector<pair<uint32_t, uint32_t>>& channels =
        active_instances[id_counter].peripheral->init_data.input_capture_channels;
...
void InputCapture::turn_on(uint8_t id) {
    ...
    if (HAL_TIM_IC_Start_IT(instance.peripheral->handle, instance.channel_rising) != HAL_OK) {
        ErrorHandler(
            "Unable to start the %s Input Capture measurement in interrupt mode",
    ...
    if (HAL_TIM_IC_Start(instance.peripheral->handle, instance.channel_falling) != HAL_OK) {
        ErrorHandler(
            "Unable to start the %s Input Capture measurement",
            instance.peripheral->name.c_str()
        );
    }
}
...
void InputCapture::turn_off(uint8_t id) {
    ...
    if (HAL_TIM_IC_Stop_IT(instance.peripheral->handle, instance.channel_rising) != HAL_OK) {
        ErrorHandler(
            "Unable to stop the %s Input Capture measurement in interrupt mode",
            instance.peripheral->name.c_str()
        );
    }
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef* htim) {
    ...
    if (rising_value != 0) {
        float ref_clock =
            HAL_RCC_GetPCLK1Freq() * 2 / (instance.peripheral->handle->Init.Prescaler + 1);
        ...
        InputCapture::active_instances[instance.id].duty_cycle =
            round((falling_value * 100) / rising_value);
    }
}
