...
map<Pin, IC::instance_constructor_data> IC::instances_data = {
...
optional<uint8_t> IC::register_ic(Pin& pin){
	...
	IC::id_manager.pop_front();
 	...
 	IC::instance_constructor_data data = IC::instances_data[pin];
	IC::instances[id] = IC::Instance(pin, data.timer,
			data.channel_rising, data.channel_falling);
	return id;
}
...
void IC::turn_on_ic(uint8_t id){
	...
	HAL_TIM_IC_Start_IT(instance.timer, instance.channel_falling);
}

void IC::turn_off_ic(uint8_t id){
	IC::Instance instance = IC::instances[id];
	HAL_TIM_IC_Stop_IT(instance.timer, instance.channel_rising);
	HAL_TIM_IC_Stop_IT(instance.timer, instance.channel_falling);
}
...
{
	uint32_t& active_channel = IC::channel_dict[htim->Channel];
	IC::Instance instance = IC::find_instance_by_channel(active_channel);

	// Rising edge skip error handle
	if (active_channel == instance.channel_rising && instance.count % 2 == 1) {
		instance.count = 0;
		return;
	}
	// Falling edge skip error handle
	if (active_channel == instance.channel_falling && instance.count % 2 == 0) {
		instance.count = 0;
		return;
	}
	...
	switch (instance.count) {
		case 0: // First Rising edge
			instance.counter_values[0] = HAL_TIM_ReadCapturedValue(instance.timer, instance.channel_rising);
			instance.count++;
			break;
		case 1: // First Falling edge
			instance.counter_values[1] = HAL_TIM_ReadCapturedValue(instance.timer, instance.channel_falling);
			instance.count++;
			break;
		case 2: // Second rising edge
			instance.counter_values[2] = HAL_TIM_ReadCapturedValue(instance.timer, instance.channel_rising);
			instance.count++;
			break;
		case 3: // Second falling edge (Redundant)
			instance.counter_values[3] = HAL_TIM_ReadCapturedValue(instance.timer, instance.channel_falling);

			//Reference clock of the IC timer calculation by dividing the clk frequency by the timer prescaler.
			uint32_t refClock = HAL_RCC_GetPCLK1Freq()*2 / (instance.timer->Init.Prescaler+1);
			uint32_t diff;

			if (instance.counter_values[2] > instance.counter_values[0]) {
				diff = instance.counter_values[2] - instance.counter_values[0];
			}
			// Overflow handling
			else if (instance.counter_values[0] > instance.counter_values[2]) {
				diff = (IC_OVERFLOW - instance.counter_values[0]) + instance.counter_values[2];
			}

			// diff/refClock gives the real time between the two edges, so refClock/diff gives freq.
			instance.frequency = refClock/diff;

			float duty_diff;
			if (instance.counter_values[1] > instance.counter_values[0]) {
				duty_diff = instance.counter_values[1] - instance.counter_values[0];
			}
			// Overflow handling
			else if (instance.counter_values[1] > instance.counter_values[2]) {
				duty_diff = (IC_OVERFLOW - instance.counter_values[0]) + instance.counter_values[1];
			}
			// duty_diff represents the amount of time the pin was high, dividing it between the whole period gives the percentage.
			instance.duty_cycle = duty_diff / diff * 100;
			instance.count = 0;
			break;
	}
}
