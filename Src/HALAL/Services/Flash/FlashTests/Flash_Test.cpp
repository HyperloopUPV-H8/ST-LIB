...
namespace FlashTest{
	...
	bool test1_writing_1_float(){
		...
		float_2_bytes(float_byte_array, NUMERTIC_PI);
	...
	bool test2_reading_1_float(){
		...
		return result * 10000 == NUMERTIC_PI * 10000;
	}
   ...
   bool	test3_writing_multiple_float(){
	   ...
	   bool first_write = first_result * 10000 == NUMERTIC_PI * 10000;

	   Flash::read(addr + offset1, (uint32_t *)float_byte_array, 1);
	   float second_result = bytes_2_float(float_byte_array);
	   bool second_write = second_result * 100000 == NUMERTIC_PI * 100000;
