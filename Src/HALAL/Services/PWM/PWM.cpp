...
map<uint8_t, PWM::Instance> PWM::active_instances = {};

PWM::TimerInitData::TimerInitData(TIM_TypeDef* timer, GPIO_TypeDef* gpio_port, uint32_t prescaler, uint32_t period) :
...
void PWM::init(TimerPeripheral& peripheral) {
...
bool PWM::instance_exists(uint8_t id) {
	if (PWM::active_instances.contains(id)) {
		return true;
	}
...
PWM::Instance& PWM::get_instance(uint8_t id) {
	return PWM::active_instances[id];
}
