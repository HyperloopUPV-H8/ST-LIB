...
void MultiplierAccelerator::IIR_software_in_software_out_inscribe(
    ...
    if (Instance.mode != MultiplierAccelerator::None) {
        ...
        if (Instance.mode == IIR) {
            ...
            sFmacConfig.CoeffBufferSize =
                MemoryLayout.FFiltersSize + MemoryLayout.FeedbackFilterSize;
            ...
            sFmacConfig.OutputBaseAddress =
                MemoryLayout.FFiltersSize + MemoryLayout.FeedbackFilterSize + MemoryLayout.FInSize;
...
void MultiplierAccelerator::software_preload(
...
void MultiplierAccelerator::software_load_repeat(int16_t* input_data, uint16_t* input_size) {
    ...
    if (HAL_FMAC_ConfigFilterOutputBuffer(
            Instance.hfmac,
...
void MultiplierAccelerator::software_load_full(
    int16_t* input_data,
    uint16_t* input_size,
    int16_t* output_data,
...
void HAL_FMAC_OutputDataReadyCallback(FMAC_HandleTypeDef* hfmac) {
    SCB_CleanInvalidateDCache_by_Addr(
        (uint32_t*)MultiplierAccelerator::Process.running_output_data,
        sizeof(MultiplierAccelerator::Process.output_data)
    );
