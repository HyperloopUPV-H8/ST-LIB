...
map<DoublePin, DoubleTimerChannel> Encoder::pin_timer_map = {
		{{&PE9, &PE11}, {&htim1, TIM_CHANNEL_1, TIM_CHANNEL_2}},
};
...
void Encoder::turn_on_encoder(uint8_t id){
	...
	DoublePin doublepin = {pin1, pin2};
	DoubleTimerChannel timer = Encoder::pin_timer_map[doublepin];
	...
	if (HAL_TIM_Encoder_Start(timer.timer, timer.channel2) != HAL_OK ||
		HAL_TIM_Encoder_Start(timer.timer, timer.channel1) != HAL_OK	) {
...
void Encoder::turn_off_encoder(uint8_t id){
	...
	if (HAL_TIM_Encoder_Stop(timer.timer, timer.channel2) != HAL_OK ||
		HAL_TIM_Encoder_Stop(timer.timer, timer.channel1) != HAL_OK	) {
