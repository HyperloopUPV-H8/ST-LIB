...
map<Pin, TIM_HandleTypeDef*> Encoder::pin_timer_map = {}; //TODO: Fill this map
map<uint8_t,pair<Pin, Pin>> Encoder::service_IDs = {};

optional<uint8_t> register_encoder(Pin& pin1, Pin& pin2){
	if (Encoder::pin_timer_map.find(pin1) == Encoder::pin_timer_map.end() ||
		Encoder::pin_timer_map.find(pin2) == Encoder::pin_timer_map.end())
    return {};
...
void turn_on_encoder(uint8_t id){
	Pin pin = Encoder::service_IDs[id].first;
	Pin pin = Encoder::service_IDs[id].second;

	TimerChannel timerChannel = Encoder::pin_timer_tap[pin];
...
void get_encoder_value(uint8_t id){
