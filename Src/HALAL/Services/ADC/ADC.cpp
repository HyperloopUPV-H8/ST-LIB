...
ADC::InitData::InitData(
    ...
    : adc(adc), resolution(resolution), external_trigger(external_trigger), channels(channels),
      dma_instance(dma_instance), name(name) {}
...
uint8_t ADC::inscribe(Pin pin) {
    if (not available_instances.contains(pin)) {
        ErrorHandler(
            "Pin %s is already used or isn t available for ADC usage",
            pin.to_string().c_str()
        );
        return 0;
    }
...
void ADC::turn_on(uint8_t id) {
    ...
    if (HAL_ADC_Start_DMA(
        ...
        ErrorHandler(
            "DMA - %d - of ADC - %d - did not start correctly",
            peripheral->init_data.dma_instance,
            id
        );
        return;
    }
    ...
    if (HAL_LPTIM_TimeOut_Start_IT(&timer.handle, timer.period, timer.period / 2) != HAL_OK) {
        ErrorHandler(
            "LPTIM - %d - of ADC - %d - did not start correctly",
            timer.name,
            peripheral->handle
        );
        return;
...
void ADC::init(Peripheral& peripheral) {
    ...
    if (adc_handle.Instance == ADC1) {
        if (HAL_ADCEx_MultiModeConfigChannel(&adc_handle, &multimode) != HAL_OK) {
            ErrorHandler(
                "ADC MultiModeConfigChannel - %d - did not start correctly",
                init_data.name
            );
            return;
        }
    ...
    __HAL_LINKDMA(&adc_handle, DMA_Handle, init_data.dma_instance->dma);
