...
void ADC::turn_on_adc(uint8_t id){
	...
	if (adcChannel.adc == &hadc1){
		HAL_ADC_Start_DMA(adcChannel.adc, adc_buf1, sizeof(adc_buf1) / sizeof(uint32_t));
	}
	else if (adcChannel.adc == &hadc2){
		HAL_ADC_Start_DMA(adcChannel.adc, adc_buf2, sizeof(adc_buf2) / sizeof(uint32_t));
	}
	else if (adcChannel.adc == &hadc3){
		HAL_ADC_Start_DMA(adcChannel.adc, adc_buf3, sizeof(adc_buf3) / sizeof(uint32_t));
	}
}
