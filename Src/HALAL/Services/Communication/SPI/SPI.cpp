...
optional<SPIPacket> SPI::get_next_packet(uint8_t id, uint16_t data_size){
	...
	if (spi.rx_queue.empty()) {
		SPI::try_receive_next_packet(id, data_size);
		return nullopt;
	}

	SPIPacket result = spi.rx_queue.front();
	spi.rx_queue.pop();

	return result;
}

void SPI::try_receive_next_packet(uint8_t id, uint16_t data_size){
	if (!SPI::registered_spi.contains(id))
		return; //TODO: Handle exception if needed
   ...
   // spi.rx_temporal_buffer = malloc(data_size);
    //TODO: Cuando llegue el packet en el callback es cuado creo el paquete
	if (HAL_SPI_Receive_DMA(spi.hspi, spi.rx_temporal_buffer, data_size) != HAL_OK) {
		return; //TODO: Handle exception if needed
	}

	spi.set_rx_status(false);
}
//TODO: Hay que ver como se gestiona el tema de que tienes que pasarle un puntero a la funcion
//		HAL_SPI_RECEIVE, seguramente lo mejor es hacer o allocar memoria dinamicamente
// 		o hacer que el buffer tenga un tamaño x fijo, preguntar que es mejor
void HAL_SPI_RxCpltCallback(SPI_HandleTypeDef *hspi){
	for (auto i: SPI::registered_spi){
		...
		if (spi.hspi == hspi) {
			...
			spi.rx_queue.push(SPIPacket(spi.rx_temporal_buffer, spi.rx_temporal_buffer_size));
			//free(spi.rx_temporal_buffer);
			return;
		}
	}
}
