...
Socket::Socket(Socket&& other)
    : connection_control_block(move(other.connection_control_block)),
      remote_port(move(remote_port)), state(other.state) {
...
void Socket::send() {
    ...
    while (error == ERR_OK && !tx_packet_buffer.empty() &&
           tx_packet_buffer.front()->len <= tcp_sndbuf(socket_control_block)) {
...
err_t Socket::poll_callback(void* arg, struct tcp_pcb* client_control_block) {
    ...
    if (socket != nullptr) {
        ...
        if (socket->state == CLOSING) {
            ...
            tcp_connect(
                socket->connection_control_block,
                &socket->remote_ip.address,
                socket->remote_port,
                connect_callback
            );
        }
...
void Socket::error_callback(void* arg, err_t error) {
    ...
    socket->close();
    ErrorHandler(
        "Client socket error: %d. Socket closed, remote ip: %s",
        error,
        socket->remote_ip.string_address.c_str()
    );
}
