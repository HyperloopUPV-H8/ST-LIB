...
void RotationComputer::cos(int32_t* angle, int32_t* out, int32_t size) {
    if (RotationComputer::mode != COSINE) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, COSINE_CONFIG);
    }
...
void RotationComputer::sin(int32_t* angle, int32_t* out, int32_t size) {
    if (RotationComputer::mode != SINE) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, SINE_CONFIG);
    }
    for (int n = 0; n < size; n++) {
        CORDIC->WDATA = angle[n];
        out[n] = CORDIC->RDATA;
...
void RotationComputer::cos_and_sin(
    int32_t* angle,
    int32_t* cos_out,
    int32_t* sin_out,
    int32_t size
    ...
    if (RotationComputer::mode != SINE_COSINE) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, SINE_COSINE_CONFIG);
    }
    for (int n = 0; n < size; n++) {
        CORDIC->WDATA = angle[n];
...
void RotationComputer::phase(int32_t* x, int32_t* y, int32_t* angle_out, int32_t size) {
    if (RotationComputer::mode != PHASE) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, PHASE_CONFIG);
    }
    for (int n = 0; n < size; n++) {
        CORDIC->WDATA = x[n];
        CORDIC->WDATA = y[n];
...
void RotationComputer::modulus(int32_t* x, int32_t* y, int32_t* out, int32_t size) {
    if (RotationComputer::mode != MODULUS) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, MODULUS_CONFIG);
    }
    for (int n = 0; n < size; n++) {
        CORDIC->WDATA = x[n];
        CORDIC->WDATA = y[n];
...
void RotationComputer::phase_and_modulus(
    int32_t* x,
    int32_t* y,
    int32_t* angle_out,
    int32_t* mod_out,
    ...
    if (RotationComputer::mode != PHASE_MODULUS) {
        ...
        MODIFY_REG(CORDIC->CSR, RESET_MASK, PHASE_MODULUS_CONFIG);
    }
    for (int n = 0; n < size; n++) {
        CORDIC->WDATA = x[n];
        CORDIC->WDATA = y[n];
