...
void DigitalOutputService::turn_off(uint8_t id){
	...
	HAL_GPIO_WritePin((GPIO_TypeDef*)pin.port, pin.gpio_pin, (GPIO_PinState)PinState::OFF);
}
...
void DigitalOutputService::turn_on(uint8_t id){
	...
	Pin pin = DigitalOutputService::service_ids[id];
	HAL_GPIO_WritePin((GPIO_TypeDef*)pin.port, pin.gpio_pin, (GPIO_PinState)PinState::ON);
}
...
void DigitalOutputService::set_pin_state(uint8_t id, PinState state){
	...
	Pin pin = DigitalOutputService::service_ids[id];
	HAL_GPIO_WritePin((GPIO_TypeDef*)pin.port, pin.gpio_pin, (GPIO_PinState) state);
}
