...
map<Pin, EXTIdata> ExtI::pin_gpio_map = {
		{PE7, {GPIO_PIN_7}
		}
};
void HAL_GPIO_EXTI_Callback (uint16_t GPIO_Pin)
{
	for(auto i : ExtI::pin_gpio_map) {
...
optional<uint8_t> ExtI::register_exti(Pin& pin, Action* action) {
...
void ExtI::unregister_exti(uint8_t id){
	...
	ExtI::id_manager.push_front(id);
}

void ExtI::turn_off_exti(uint8_t id){
	Pin pin = ExtI::service_ids[id];
	EXTIdata exti_data = ExtI::pin_gpio_map[pin];
...
bool ExtI::get_pin_value(uint8_t id) {
	Pin pin = ExtI::service_ids[id];
