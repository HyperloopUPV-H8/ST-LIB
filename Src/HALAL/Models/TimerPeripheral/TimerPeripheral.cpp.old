...
TimerPeripheral::InitData::InitData(
    ...
    : type(type), prescaler(prescaler), period(period), deadtime(deadtime), polarity(polarity),
      negated_polarity(negated_polarity) {}

TimerPeripheral::TimerPeripheral(TIM_HandleTypeDef* handle, InitData&& init_data, string name)
    : handle(handle), init_data(init_data), name(name) {}
...
void TimerPeripheral::init() {
    ...
    for (pair<uint32_t, uint32_t>& channels_rising_falling : init_data.input_capture_channels) {
        ...
        if (HAL_TIM_IC_ConfigChannel(handle, &sConfigIC, channels_rising_falling.second) !=
            HAL_OK) {
            ErrorHandler("Unable to configure a IC channel on %d", name.c_str());
        }
    }
...
uint32_t TimerPeripheral::get_prescaler() { return handle->Instance->PSC; }

uint32_t TimerPeripheral::get_period() { return handle->Instance->ARR; }
