...
template <class... Types> struct has_callable;

template <class Type, class... Types> struct has_callable<Type, Types...> {
    static constexpr bool value = Callable<Type> || has_callable<Types...>::value;
};

template <> struct has_callable<> {
...
template <class Type, class... Types> struct has_container<Type, Types...> {
public:
    static constexpr bool value = Container<Type> || has_container<Types...>::value;
};

template <> struct has_container<> {
public:
    static constexpr bool value = false;
};
...
template <class Type, class... Types> struct total_sizeof<Type, Types...> {
public:
    static constexpr size_t value = sizeof(Type) + total_sizeof<Types...>::value;
};

template <> struct total_sizeof<> {
public:
...
template <class Type> class FollowingUint;

template <> class FollowingUint<uint8_t> {
public:
    using Value = uint16_t;
};

template <> class FollowingUint<uint16_t> {
public:
    using Value = uint32_t;
};

template <> class FollowingUint<uint32_t> {
public:
