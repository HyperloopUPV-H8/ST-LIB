...
template <size_t BufferLength, class... Types> class StackPacket : public Packet {
public:
    uint16_t id;
    ...
    StackPacket(uint16_t id, Types*... values)
        : id(id), packetvalue_warehouse{PacketValue<Types>(values)...} {
        int i = 0;
    ...
    void set_pointer(size_t index, void* pointer) override { values[index]->set_pointer(pointer); }
};

template <class... Types> class StackPacket<0, Types...> : public Packet {
public:
    uint16_t id;
    PacketValue<>* values[sizeof...(Types)];
    stack_tuple<PacketValue<Types>...> packetvalue_warehouse;
...
template <> class StackPacket<0> : public Packet {
public:
    uint16_t id;
    ...
    void parse(uint8_t* data) override {}
    uint8_t* build() override { return (uint8_t*)&id; }
    uint16_t get_id() override { return id; }
    size_t get_size() override { return sizeof(id); }
...
StackPacket(uint16_t, Types*... values)
    -> StackPacket<(!has_container<Types...>::value) * total_sizeof<Types...>::value, Types...>;
...
public:
    ...
    template <class... Types> HeapPacket(uint16_t id, Types*... values) : id(id) {
