 ...
 */
    ...
    enum class ClockGroup : uint8_t {
        SPI123_G, SPI45_G, SPI6_G, SDMMC_G, ADC_G, FDCAN_G,
        USART16_G, USART234578_G, I2C1235_G, I2C4_G,
        LPUART1_G, LPTIM_G, RNG_G, USB_G, DFSDM1_G, SAI1_G, SAI4_G,
        ETH_MAC_G, RTC_G, CEC_G,
        APB1_TIM_G, APB2_TIM_G, D1_BUS_G, D2_BUS_G, D3_BUS_G,
    };
    ...
    static consteval bool is_valid_pll1p(uint32_t p) {
        return p == PLLP_MIN || (p <= PLLP_MAX && p % 2 == 0); // 1 is bypass, then even numbers up to 128
    }
    ...
    static constexpr uint32_t find_rge(uint32_t pll_input) {
        if (pll_input >= 1'000'000 && pll_input <= 2'000'000) return 0;
        if (pll_input >  2'000'000 && pll_input <= 4'000'000) return 1;
        if (pll_input >  4'000'000 && pll_input <= 8'000'000) return 2;
        if (pll_input >  8'000'000 && pll_input <= 16'000'000) return 3;
        return 0;
    }

    static constexpr uint32_t flash_ws(uint32_t hclk) {
        if (hclk <= 70'000'000)  return 1;
        if (hclk <= 140'000'000) return 2;
        if (hclk <= 210'000'000) return 3;
        return 4;
    }
    ...
    static constexpr uint32_t timer_apb1(const ClockTree& t) { return pclk1(t) * (t.d2ppre1 == 1 ? 1 : 2); }
    static constexpr uint32_t timer_apb2(const ClockTree& t) { return pclk2(t) * (t.d2ppre2 == 1 ? 1 : 2); }
    ...
    static constexpr uint32_t source_frequency(const ClockTree& t, ClockTree::Source src) {
        switch (src) {
            case ClockTree::Source::HSI:   return 64'000'000;
            case ClockTree::Source::HSE:   return t.hse_frequency;
            case ClockTree::Source::CSI:   return 4'000'000;
            case ClockTree::Source::PCLK1: return pclk1(t);
            case ClockTree::Source::PCLK2: return pclk2(t);
            case ClockTree::Source::PLL1Q: return pll1_vco(t) / t.pll1_q;
            case ClockTree::Source::PLL1R: return pll1_vco(t) / t.pll1_r;
            case ClockTree::Source::PLL2P: return t.pll2_p != 0 ? pll2_vco(t) / t.pll2_p : 0;
            case ClockTree::Source::PLL2Q: return t.pll2_q != 0 ? pll2_vco(t) / t.pll2_q : 0;
            case ClockTree::Source::PLL2R: return t.pll2_r != 0 ? pll2_vco(t) / t.pll2_r : 0;
            case ClockTree::Source::PLL3P: return t.pll3_p != 0 ? pll3_vco(t) / t.pll3_p : 0;
            case ClockTree::Source::PLL3Q: return t.pll3_q != 0 ? pll3_vco(t) / t.pll3_q : 0;
            case ClockTree::Source::PLL3R: return t.pll3_r != 0 ? pll3_vco(t) / t.pll3_r : 0;
            default: break;
        }
        if consteval {
    ...
    static constexpr uint32_t get_kernel_clock(const ClockTree& t, ClockGroup group) {
        switch (group) {
            case ClockGroup::SPI123_G:   return source_frequency(t, t.spi123_src);
            case ClockGroup::SPI45_G:    return source_frequency(t, t.spi45_src);
            case ClockGroup::SPI6_G:   return source_frequency(t, t.spi6_src);
            case ClockGroup::ADC_G:      return source_frequency(t, t.adc_src);
            case ClockGroup::FDCAN_G:    return source_frequency(t, t.fdcan_src);
            case ClockGroup::SDMMC_G:    return source_frequency(t, t.sdmmc_src);
            case ClockGroup::APB1_TIM_G: return timer_apb1(t);
            case ClockGroup::APB2_TIM_G: return timer_apb2(t);
            case ClockGroup::D1_BUS_G:   return sysclk(t);
            case ClockGroup::D2_BUS_G:   return pclk1(t);
            case ClockGroup::D3_BUS_G:   return pclk2(t);
            default: break;
        }
        if consteval {
 ...
 */
    ...
    static consteval void validate(const ClockTree& t, std::span<const Entry> entries) {
        if (t.hse_frequency == 0) compile_error("HSE frequency is zero");
        if (t.pll1_m == 0) compile_error("PLL1M is zero");
        ...
        if (sclk > SYSCLK_MAX) compile_error("SYSCLK exceeds maximum (550 MHz)");
        if (h_clk > HCLK_MAX) compile_error("HCLK exceeds maximum (275 MHz)");
        if (pll_in < PLL_IN_MIN || pll_in > PLL_IN_MAX)
            compile_error("PLL1 input outside valid range (1-16 MHz)");
        if (t.pll1_m < PLLM_MIN || t.pll1_m > PLLM_MAX) compile_error("PLL1M out of range");
        if (t.pll1_n < PLLN_MIN || t.pll1_n > PLLN_MAX) compile_error("PLL1N out of range");
        if (!is_valid_pll1p(t.pll1_p)) compile_error("Invalid PLL1P divider");
        ...
        for (auto d : d1cpre_values) {
            if (t.d1cpre == d) { d1cpre_valid = true; break; }
        }
        if (!d1cpre_valid) compile_error("D1CPRE is not a valid divider");

        // Peripheral requirements
        for (size_t i = 0; i < entries.size(); i++) {
            const auto& ent = entries[i];
            uint32_t ker = get_kernel_clock(t, ent.group);
            if (ker == 0) compile_error("No kernel clock assigned for peripheral group");
    ...
    static constexpr uint32_t to_hal_rge(uint32_t rge) {
        switch (rge) {
            case 0: return RCC_PLL1VCIRANGE_0;
            case 1: return RCC_PLL1VCIRANGE_1;
            case 2: return RCC_PLL1VCIRANGE_2;
            case 3: return RCC_PLL1VCIRANGE_3;
            default: return RCC_PLL1VCIRANGE_0;
        }
    }
    ...
    static constexpr uint32_t to_hal_apb_pre(uint32_t d) {
        switch (d) {
            case 1:  return RCC_APB1_DIV1;
            case 2:  return RCC_APB1_DIV2;
            case 4:  return RCC_APB1_DIV4;
            case 8:  return RCC_APB1_DIV8;
            case 16: return RCC_APB1_DIV16;
            default: return RCC_APB1_DIV2;
        }
    }
    ...
    static void apply_system_clocks(const ClockTree& t) {
        ...
        __HAL_PWR_VOLTAGESCALING_CONFIG(
            sysclk(t) > 480'000'000 ? PWR_REGULATOR_VOLTAGE_SCALE0
    ...
    static void apply_peripheral_clocks(const ClockTree& t) {
        ...
        switch (t.spi123_src) {
            case ClockTree::Source::PLL1Q: pclk.Spi123ClockSelection = RCC_SPI123CLKSOURCE_PLL; break;
            case ClockTree::Source::PLL2P: pclk.Spi123ClockSelection = RCC_SPI123CLKSOURCE_PLL2; break;
            case ClockTree::Source::PLL3P: pclk.Spi123ClockSelection = RCC_SPI123CLKSOURCE_PLL3; break;
            case ClockTree::Source::HSI:   pclk.Spi123ClockSelection = RCC_SPI123CLKSOURCE_CLKP; break;
            default: PANIC("ClockDomain: SPI123 clock source not supported"); break;
        }

        pclk.PeriphClockSelection |= RCC_PERIPHCLK_SPI4;
        switch (t.spi45_src) {
            case ClockTree::Source::PCLK2: pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_D2PCLK2; break;
            case ClockTree::Source::PLL2Q: pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_PLL2; break;
            case ClockTree::Source::PLL3Q: pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_PLL3; break;
            case ClockTree::Source::HSI:   pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_HSI; break;
            case ClockTree::Source::CSI:   pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_CSI; break;
            case ClockTree::Source::HSE:   pclk.Spi45ClockSelection = RCC_SPI45CLKSOURCE_HSE; break;
            default: PANIC("ClockDomain: SPI45 clock source not supported"); break;
        }

        pclk.PeriphClockSelection |= RCC_PERIPHCLK_SPI6;
        switch (t.spi6_src) {
            case ClockTree::Source::PCLK2: pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_D3PCLK1; break;
            case ClockTree::Source::PLL2P: pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_PLL2; break;
            case ClockTree::Source::PLL3P: pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_PLL3; break;
            case ClockTree::Source::HSI:   pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_HSI; break;
            case ClockTree::Source::CSI:   pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_CSI; break;
            case ClockTree::Source::HSE:   pclk.Spi6ClockSelection = RCC_SPI6CLKSOURCE_HSE; break;
            default: PANIC("ClockDomain: SPI6 clock source not supported"); break;
        }

        pclk.PeriphClockSelection |= RCC_PERIPHCLK_SDMMC;
        switch (t.sdmmc_src) {
            case ClockTree::Source::PLL1Q: pclk.SdmmcClockSelection = RCC_SDMMCCLKSOURCE_PLL; break;
            case ClockTree::Source::PLL2R: pclk.SdmmcClockSelection = RCC_SDMMCCLKSOURCE_PLL2; break;
            default: PANIC("ClockDomain: SDMMC clock source not supported"); break;
        }
        ...
        switch (t.adc_src) {
            case ClockTree::Source::HSE:   pclk.AdcClockSelection = RCC_ADCCLKSOURCE_CLKP; break;
            case ClockTree::Source::PLL2R: pclk.AdcClockSelection = RCC_ADCCLKSOURCE_PLL2; break;
            case ClockTree::Source::PLL3R: pclk.AdcClockSelection = RCC_ADCCLKSOURCE_PLL3; break;
            default: PANIC("ClockDomain: ADC clock source not supported"); break;
        }

        pclk.PeriphClockSelection |= RCC_PERIPHCLK_FDCAN;
        switch (t.fdcan_src) {
            case ClockTree::Source::PLL2Q: pclk.FdcanClockSelection = RCC_FDCANCLKSOURCE_PLL2; break;
            case ClockTree::Source::PLL1Q: pclk.FdcanClockSelection = RCC_FDCANCLKSOURCE_PLL; break;
            case ClockTree::Source::HSE:   pclk.FdcanClockSelection = RCC_FDCANCLKSOURCE_HSE; break;
            default: PANIC("ClockDomain: FDCAN clock source not supported"); break;
        }
