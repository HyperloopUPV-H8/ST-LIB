...
#define DualPWMmap                                                                                 \
    map<pair<Pin, Pin>, pair<reference_wrapper<TimerPeripheral>, TimerPeripheral::PWMData>>

class TimerPeripheral {
public:
    enum PWM_MODE : uint8_t { NORMAL = 0, PHASED = 1, CENTER_ALIGNED = 2 };

    enum TIM_TYPE { BASE, ADVANCED };
    ...
    public:
        ...
        InitData(
            ...
            uint32_t period = 55000,
            uint32_t deadtime = 0,
            uint32_t polarity = TIM_OCPOLARITY_HIGH,
            uint32_t negated_polarity = TIM_OCPOLARITY_HIGH
        );
    };
