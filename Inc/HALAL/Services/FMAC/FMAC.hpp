...
public:
     ...
     */
    static void IIR_software_in_software_out_inscribe(
        uint16_t input_coefficient_array_size,
        int16_t* input_coefficient_array,
        uint16_t feedback_coefficient_array_size,
    ...
    /**
     * @brief preloads data to be run into the FMAC. Recommended to preload at least as much zeroes
     * as Feedback Coefficients in the feedback data if no data is planned to be preloaded.
     */
    static void software_preload(
        int16_t* preload_data,
        uint8_t amount_to_preload,
        int16_t* preload_feedback_data,
        uint8_t amount_to_feedback_preload
    );

    /**
     * @brief orders the FMAC to start processing, and will run calculations with the preloaded
     * data.
     */
     ...
     */
    static void software_load_full(
        int16_t* input_data,
        uint16_t* input_size,
        int16_t* output_data,
    ...
    /**
     * @brief wether or not the FMAC is ready to receive more data. Should be called before using
     * any load function to check if the peripheral is ready.
     */
...
private:
     ...
     *
     * @param FeedbackFiltersCoeffs pointer to the Coefficients used in the Feedback mode. Declare
     * them as nullptr on FIR as they will not be used. Known as A coefficients in FMAC HAL
     * peripheral
     * @param FeedbackFilterSize number of FeedbackCoefficients, must be one less than FFiltersSize
     * in normal Feedback use, and 0 on any FIR use
     * @param FFiltersCoeffs pointer to the Coefficients used in all modes as the external input.
     * @param FFiltersSize number of FIRCoefficients, equal to the inputs computated in one
     * calculation
     * @param
     */
    static void configure(
        int16_t* FeedbackFiltersCoeffs,
        uint8_t FeedbackFilterSize,
        int16_t* FFiltersCoeffs,
        uint8_t FFiltersSize,
        int16_t* FeedbackInValues,
        uint8_t FeedbackInSize,
        int16_t* FInValues,
        uint8_t FInSize,
        int16_t* OutValues,
        uint8_t OutSize,
