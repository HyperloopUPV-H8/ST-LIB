...
#define MASTER_SPI_CHECK_DELAY                                                                     \
    100000 // how often the master should check if the slave is ready, in nanoseconds

#define MASTER_MAXIMUM_QUEUE_LEN 10

// TODO: Hay que hacer el Chip select funcione a traves de un GPIO en vez de a traves del
// periferico.
...
public:
     ...
     * NOT intended to be modified after the inscribe of the spi, configuration should be made in
     * Runes.hpp
     *
     */
    struct Instance {
        ...
        uint8_t* tx_buffer;
        uint16_t*
            available_end;    /**< variable that checks for what Order id is the other end ready*/
        uint16_t* SPIOrderID; /**< SPIOrder being processed, if any*/
        RingBuffer<uint16_t, MASTER_MAXIMUM_QUEUE_LEN>
            SPIOrderQueue;           /**< Queue of SPIOrders to process after this one*/
        ...
        uint64_t try_count =
            0; /**< Tries from the master to communicate a packet with the slave for debugging
                  (affected by how much the slave delays on preparing a packet)*/
        uint64_t error_count = 0; /**< Order error counter for debugging (affected by how much the
                                     bits sent are corrupted in any way)*/
    };

    /**
     * @brief Enum that abstracts the use of the Instance struct to facilitate the mocking of the
     * HALAL.
     *
     */
    enum Peripheral {
     ...
     */
    static bool
    transmit_and_receive(uint8_t id, span<uint8_t> command_data, span<uint8_t> receive_data);
    ...
    /**
     * @brief slave listen Orders method. When called, the slave will start to listen Orders until
     * the state is set again to IDLE
     */
     ...
     */
    static bool
    transmit_and_receive_DMA(uint8_t id, span<uint8_t> command_data, span<uint8_t> receive_data);
    /**
     * @brief method that needs to be called periodically by the master code for proper
     * communication. Can be called at any speed, but the faster it is called, the faster the Orders
     * will resolve.
     *
     * this update has to be called by the code in order for master to check if the slave is ready
     * to send the Order. If it is not called periodically, the master_transmit_Order will not work.
     * Not needed for dummy communication (not using Orders)
     */
    ...
    /**
     * @brief master Order that checks the state of the slave, used on the callback of the SPI
     * module and handled automatically by Order_update method (as long as it is called).
     */
    static void master_check_available_end(SPI::Instance* spi);

    /**
     * @brief slave updates the information on its buffer to check what ID is master asking on next
     * query
     */
    static void slave_check_packet_ID(SPI::Instance* spi);

    /**
     * @brief This method sets chip select to high level, and is used automatically by the SPI
     * module
     *
    ...
    /**
     * @brief This method sets chip select to low level, and is used automatically by the SPI
     * module.
     *
     * The low level signal on SPI on chip select marks the slave connected to it to start
     * processing data, and is marked by the master The slave on the ST-LIB doesn t use this
     * configuration yet and is instead active all the time, so for now it can only function as a
     * single point to point configuration to have multiple ST-LIB SPI slaves on a single SPI
     * interface you may need to implement an EXTI on the slave that deactivates it while its on.
     * This method is still used to communicate to any slave that is not coded with the ST-LIB, as
     * those follow the common rules of the SPI
     *
     * @param spi Id of the SPI
     */
    static void chip_select_off(uint8_t id);

    static inline void spi_communicate_order_data(
        SPI::Instance* spi,
    ...
    /*
     * @brief returns true if its KNOWN that the slave has the packet ready, and false if it is NOT
     * KNOWN
     *
     * This function uses the optional RS to know if the slave is ready, and always returns false if
     * the RS is not used.
     */
