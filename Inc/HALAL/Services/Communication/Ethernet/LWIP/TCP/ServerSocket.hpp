...
public:
    ...
    /**
     * @brief ServerSocket constructor that receives the server ip on the net as a
     * binary value.
     *
    ...
    /**
     * @brief puts the order data into the tx_packet_buffer and sends all the data
     ...
     * sending one by one the packets in the tx_packet_buffer The messages in the
     * buffer are all immediately sent after calling this function, unless an
     * error of any kind happened, in which case ErrorHandler is raised
     */
    void send();
...
private:
     ...
     * called only by Lwip on the receive_callback
     *
     * reads all the data received by the server in the ethernet buffer, packet by
     * packet. Then, for each packet, it processes it depending on the order id
     * (default behavior is not process) and removes it from the buffer. This
     * makes so the receive_callback (and thus the Socket) can only process
     * declared orders, and ignores all other packets.
     */
    void process_data();
    /**
     * @brief the callback for the listener socket receiving a request for
     * connection into the ServerSocket.
     *
     * This function is called on an interrupt when a packet containing a
     * connection request to the same port of the listener socket is received.
     * accept_callback builds the pcb that acts as the connection socket and saves
     * it in the client_control_block pointer It then closes the listener socket
     * and makes the server_control_block point to nullptr.
     *
     * server_control_block shouldn't be accessed in any way while the
     * ServerSocket is in the ServerSocket#ACCEPT state, as it will lead into a
     * nullptr exception. This in an intended behavior as there shouldn't be more
     * than one listener socket on the same port, and a ServerSocket shouldn't be
     * able to handle more than one connection by design.
     */
     ...
     *
     * On a received packet on an ACCEPTED state receive_callback calls the
     * ServerSocket process_data, which calls the Packet process_data() if it is
     * inscribed as an Order.
     */
     ...
     *
     * The housekeeping tasks made now are basically checks to ensure no data
     * remains unsent and that the ServerSocket is not left in a middle state
     * (such as CLOSING)
     */
